(() => {
  ...
  const cloudError = (result) => { const error=new Error(result?.message || 'Cloud sync failed.'); error.code=result?.code || 'firestore/unknown'; return error; };
  ...
  window.JNCOSInquiryStore = {
    ...
    async create(payload){
      ...
      if(window.JNCOSCloudStore?.configured){
        ...
        if(!result?.ok){ writeLocal(previous); throw cloudError(result); }
      }
      return item;
    },
    async list(){
