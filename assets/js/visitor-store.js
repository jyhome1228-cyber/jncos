(() => {
  ...
  const merge = (local, cloud) => { const map = new Map(); [...local, ...cloud].forEach((item) => { if (item?.id) map.set(item.id, { ...(map.get(item.id) || {}), ...item }); }); return [...map.values()].sort((a,b) => new Date(b.lastSeen || b.createdAt || 0) - new Date(a.lastSeen || a.createdAt || 0)); };
  ...
  const trackPageView = async () => {
    ...
    const record = existing ? { ...existing, lastSeen:now, date:indiaDate(), pageViews:(Number(existing.pageViews)||0)+1, currentPage:location.pathname } : {
      id:sessionId, visitorId, sessionId, type:'visit', createdAt:now, firstSeen:now, lastSeen:now, date:indiaDate(), pageViews:1,
      landingPage:location.pathname, currentPage:location.pathname, referrer:document.referrer || '',
      utmSource:params.get('utm_source') || '', utmMedium:params.get('utm_medium') || '', utmCampaign:params.get('utm_campaign') || '',
      language:navigator.language || '', device:/Mobi|Android/i.test(navigator.userAgent) ? 'Mobile' : 'Desktop'
    };
  ...
  window.JNCOSVisitorStore = {
    ...
    trackPageView,
    async list() { await ensureBackend(); return merge(localList(), await window.JNCOSCloudStore?.list?.('visits',1500) || []); },
    async stats() {
      const visits = await this.list();
      const today = indiaDate();
      const now = new Date();
      const since = new Date(now.getTime() - 6 * 86400000);
      const days = []; for (let i=6;i>=0;i--) days.push(indiaDate(new Date(now.getTime()-i*86400000)));
      ...
      const daily = days.map((date) => ({ date, visitors:unique(visits.filter((v)=>v.date===date)), sessions:visits.filter((v)=>v.date===date).length, pageViews:visits.filter((v)=>v.date===date).reduce((sum,v)=>sum+(Number(v.pageViews)||0),0) }));
      return { todayVisitors:unique(todayRows), weekVisitors:unique(weekRows), totalVisitors:unique(visits), totalSessions:visits.length, pageViews:visits.reduce((sum,v)=>sum+(Number(v.pageViews)||0),0), daily, recent:visits.slice(0,20) };
    }
  };
})();
